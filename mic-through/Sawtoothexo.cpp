...
struct mydsp : public dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "Sawtoothexo.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("Sawtoothexo");
		ui_interface->addHorizontalSlider("ctf", &fHslider0, FAUSTFLOAT(5e+03f), FAUSTFLOAT(1e+02f), FAUSTFLOAT(2e+04f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("freq", &fHslider2, FAUSTFLOAT(4.4e+02f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(2e+03f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("gain", &fHslider1, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = (fTemp1 + 0.14285715f) / fTemp0 + 1.0f;
			fRec2[0] = fSlow1 + 0.999f * fRec2[1];
			fRec4[0] = fSlow2 + 0.999f * fRec4[1];
			float fTemp3 = std::max<float>(fRec4[0], 23.44895f);
			float fTemp4 = std::max<float>(2e+01f, std::fabs(fTemp3));
			float fTemp5 = ((1 - iVec0[1]) ? 0.0f : fRec5[1] + fConst2 * fTemp4);
			fRec5[0] = fTemp5 - std::floor(fTemp5);
			float fTemp6 = mydsp_faustpower2_f(2.0f * fRec5[0] + -1.0f);
			fVec1[0] = fTemp6;
			float fTemp7 = static_cast<float>(iVec0[1]) * (fTemp6 - fVec1[1]) / fTemp4;
			fVec2[IOTA0 & 4095] = fTemp7;
			float fTemp8 = std::max<float>(0.0f, std::min<float>(2047.0f, fConst3 / fTemp3));
			int iTemp9 = static_cast<int>(fTemp8);
			float fTemp10 = std::floor(fTemp8);
			fRec3[0] = 0.999f * fRec3[1] + fConst4 * (fTemp7 - fVec2[(IOTA0 - iTemp9) & 4095] * (fTemp10 + (1.0f - fTemp8)) - (fTemp8 - fTemp10) * fVec2[(IOTA0 - (iTemp9 + 1)) & 4095]);
			fRec1[0] = fConst5 * fRec4[0] * fRec3[0] * fRec2[0] - (fRec1[2] * ((fTemp1 + -0.14285715f) / fTemp0 + 1.0f) + 2.0f * fRec1[1] * (1.0f - 1.0f / mydsp_faustpower2_f(fTemp0))) / fTemp2;
	...
	#define FAUST_FILE_NAME "Sawtoothexo.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("ctf", fHslider0, 5e+03f, 1e+02f, 2e+04f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider2, 4.4e+02f, 5e+01f, 2e+03f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("gain", fHslider1, 1.0f, 0.0f, 1.0f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, ctf, "ctf", fHslider0, 5e+03f, 1e+02f, 2e+04f, 0.01f) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider2, 4.4e+02f, 5e+01f, 2e+03f, 0.01f) \
		p(HORIZONTALSLIDER, gain, "gain", fHslider1, 1.0f, 0.0f, 1.0f, 0.01f) \
...
Sawtoothexo::Sawtoothexo() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void Sawtoothexo::updateImp(void)
{
#if MIDICTRL
...
void Sawtoothexo::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void Sawtoothexo::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float Sawtoothexo::getParamValue(const std::string& path)
{
